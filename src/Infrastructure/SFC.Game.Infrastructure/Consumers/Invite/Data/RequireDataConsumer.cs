    ...
    : IConsumer<RequireData>
{
    ...
    {
        GetInviteDataModel model = await _gameDataService.GetInviteDataAsync().ConfigureAwait(true);
...
public class RequireDataConsumerDefinition : ConsumerDefinition<RequireDataConsumer>
{
    ...
    private Message Exchange { get { return _settings.Exchanges.Invite.Value.Data.Dependent.Game.RequireInitialize; } }

    public RequireDataConsumerDefinition(IConfiguration configuration)
    {
        _settings = configuration.GetRabbitMqSettings();
        EndpointName = "sfc.game.invite.data.initialize.require.queue";
    }
