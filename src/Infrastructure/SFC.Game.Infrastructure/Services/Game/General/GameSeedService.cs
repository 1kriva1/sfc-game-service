...
public class GameSeedService(
    ...
    IGameRepository gameRepository) : IGameSeedService
{
#pragma warning disable CA5394 // Do not use insecure randomness
    ...
    private static readonly IList<DateOnly?> AVAILABLE_DATES = [
        DateOnly.FromDateTime(new DateTime(2026, 1, 12)),
        DateOnly.FromDateTime(new DateTime(2026, 6, 7)),
        DateOnly.FromDateTime(new DateTime(2026, 2, 22)),
        DateOnly.FromDateTime(new DateTime(2025, 1, 12)),
        DateOnly.FromDateTime(new DateTime(2036, 1, 12)),
        DateOnly.FromDateTime(new DateTime(2016, 1, 12)),
    ];
    ...
    public Task<IEnumerable<GameEntity>> GetSeedGamesAsync()
    {
        return _gameRepository.GetByUserIdsAsync(USER_IDS);
    }

    public async Task SeedGamesAsync(CancellationToken cancellationToken = default)
    {
        IEnumerable<GameEntity> games = CreateSeedGames();

        GameEntity[] seedGames = await
            _gameRepository.AddRangeIfNotExistsAsync([.. games]).ConfigureAwait(true);

        await PublishGamesSeededEventAsync(seedGames, cancellationToken).ConfigureAwait(true);

        await _metadataService.CompleteAsync(MetadataServiceEnum.Game, MetadataDomainEnum.Game, MetadataTypeEnum.Seed).ConfigureAwait(true);
    }
    ...
    private IEnumerable<GameEntity> CreateSeedGames()
    {
        IEnumerable<GameEntity> result = USER_IDS.Select(userId => BuildGame(userId));
        return result;
    }

    private GameEntity BuildGame(Guid userId)
    {
        DateTime createdDate = _dateTimeService.Now;
        string name = GetAndRemove(NAMES) ?? $"{GenerateRandomString(4)}";
        DateOnly? date = GetAndRemove(AVAILABLE_DATES);
        ...
        GameEntity game = new()
        {
            ...
            UserId = userId,
            StatusId = GetRandomEnumValue<GameStatusEnum>(),
            GeneralProfile = new GameGeneralProfile
            {
                Name = name,
                Description = ValueOrNull($"{name} has great description."),
                LocationId = null
            },
            Availability = new GameAvailability
            {
                Date = date ?? DateOnly.FromDateTime(new DateTime(2026, 12, 4)),
                From = time?.Item1 ?? new TimeSpan(18, 30, 0),
                To = time?.Item2 ?? new TimeSpan(22, 30, 0)
            },
            FinancialProfile = new GameFinancialProfile
            {
                FreeGame = GetRandomBoolean(),
                PayAmount = Random.Next(1000),
            },
            InventaryProfile = new GameInventaryProfile
            {
                ShirtsRequired = GetRandomBoolean(),
                ShirtsCount = Random.Next(10)
            }
        };

        BuildGameTags().ForEach(game.Tags.Add);

        return game;
    }

    private Task PublishGamesSeededEventAsync(IEnumerable<GameEntity> games, CancellationToken cancellationToken = default)
    {
        GamesSeeded @event = _mapper.Map<GamesSeeded>(games);
        return _publisher.Publish(@event, cancellationToken);
    }

    private static List<GameTag> BuildGameTags()
    {
        List<GameTag> result = TAGS.Take(Random.Next(TAGS.Count))
                .Select(tag => new GameTag { Value = tag })
                .ToList();

        return ValueOrNull(result) ?? [];
    }

    private static T? GetAndRemove<T>(IList<T> list)
    {
        lock (list)
        {
